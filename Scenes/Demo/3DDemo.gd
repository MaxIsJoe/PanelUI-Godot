...
onready var node_quad = $CSGMesh
onready var node_area = $CSGMesh/Area
...
func _input(event):         
	if event is InputEventMouseMotion:
		var movment = event.relative
		$Camera.rotation.x += -deg2rad(movment.y * sens)
		$Camera.rotation.x = clamp($Camera.rotation.x, deg2rad(-40), deg2rad(40))
		$Camera.rotation.y = -deg2rad(movment.x * sens)
